...
SRCdir2='ft_printf'										#	Srcs directory name
SRCdir3='get_next_line'										#	Srcs directory name
...
("$rem_old") into "$splitA" & "$splitB$RC"\n"
	...
	then
		...
		find ./$SRCdir2 -type f -name "$SRCfindptrn" | sed -e 's|^|	|'| sed -e 's|$| \\|' >> $splitA
		find ./$SRCdir3 -type f -name "$SRCfindptrn" | sed -e 's|^|	|'| sed -e 's|$| \\|' >> $splitA
